...
%In statistical genomics studies, for example in genome-wide association studies (GWAS), the transition from theoretical axiomatic variants (QV\_ax) to empirically testable variants (QV1) is a critical step. QV\_ax represents an idealized set of genetic variants that perfectly align with fundamental genetic principles, considered correct by genetic doctrine. We discuss this in detail elsewhere (cite Bayesian framwork paper). Due to current technological limitations and incomplete genetic knowledge, this set is often beyond empirical verification. Conversely, QV1 comprises variants subjected to standard GWAS pre-processing filters such as --geno, --maf, --hwe, and --mind, which select variants based on criteria like missing genotype data, minor allele frequency, Hardy-Weinberg equilibrium, and individual missing data.

% The assessment of variants from QV\_ax to QV1 involves stringent criteria to ensure the data's reliability and scientific validity. This process, however, might lead to the exclusion of potentially informative variants, classified as false negatives (FN). It also defines the true positives (TP) as those variants present in both QV\_ax and QV1. The dynamics of this filtration process and its implications can be mathematically represented as follows:
...
% These equations elucidate the relationship between axiomatic and filtered variants, quantifying the true positives and identifying the axiomatic variants erroneously excluded by the empirical filters. This analytical framework aids in understanding the trade-offs involved in GWAS pre-processing—balancing data quality against the risk of overlooking key genetic contributors to phenotypes under study.
