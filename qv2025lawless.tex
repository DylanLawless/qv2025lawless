...
We argue that standardising and optimising QVs for advanced multi-stage use, rather than limiting them to conceptually simplistic, single-stage filters, can significantly advance omics research and open up new theoretical domains. 
...
For simplicity, we can assume that each set represents one \ac{qwas}.
...
By employing \ac{acat}, we can synthesize findings from multiple QV filters applied to the same genomic dataset, leading to a more comprehensive understanding of the genetic architecture of traits under study. 
This method is particularly useful in scenarios where variants across different QV sets may contribute in varying degrees to the phenotype, allowing for a nuanced analysis that respects the complexity of genomic data.
