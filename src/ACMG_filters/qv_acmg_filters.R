...
apply_acmg_ps1 <- function(df, criteria) {
  field_name <- criteria$fields$clinical_significance
  terms <- criteria$terms
  pattern <- paste(terms, collapse = "|")
  df$ACMG_PS1 <- ifelse(grepl(pattern, df[[field_name]], ignore.case = TRUE), "PS1", NA)
  return(df)
}

df <- apply_acmg_ps1(df, acmg_criteria$ACMG_PS1)
table(df$ACMG_PS1)
...
apply_acmg_ps3_fixed <- function(df, criteria) {
  df$ACMG_PS3 <- NA

  for (cond in criteria$conditions) {
    field_name <- cond$condition$field
    operator <- cond$condition$operator
    value <- cond$condition$value
    add_field_name <- cond$condition$additional_criteria$field
    add_values <- cond$condition$additional_criteria$values

    expr <- sprintf("df[['%s']] %s %s & df[['%s']] %%in%% c(%s)",
                    field_name, operator, value, add_field_name,
                    paste(shQuote(add_values), collapse = ", "))

    print(expr)

    final_condition_result <- eval(parse(text = expr))
    df$ACMG_PS3[final_condition_result] <- "PS3"
  }

  return(df)
}

df <- apply_acmg_ps3_fixed(df, acmg_criteria$ACMG_PS3)
table(df$ACMG_PS3)
...
apply_acmg_ps5 <- function(df, criteria) {
  impact_field <- criteria$conditions[[1]]$condition$field
  impact_value <- criteria$conditions[[1]]$condition$value
  impact_operator <- criteria$conditions[[1]]$condition$operator
  group_fields <- criteria$conditions[[2]]$condition$group_by
  
  query <- paste(impact_field, impact_operator, shQuote(impact_value))
  print(paste("Query to evaluate: any(", query, ") & n() > 1"))
  
  df <- df %>%
    group_by(across(all_of(group_fields))) %>%
    mutate(ACMG_PS5 = ifelse(any(eval(parse(text = query))) & n() > 1, "PS5", ACMG_PS5)) %>%
    ungroup()
  
  return(df)
}

df <- apply_acmg_ps5(df, ps5_criteria)
print(table(df$ACMG_PS5))
...
apply_acmg_pm2 <- function(df, criteria) {
  # Initialize PM2 column to NA
  df$ACMG_PM2 <- NA
  
  # Retrieve condition details from YAML
  field_name <- criteria$conditions[[1]]$condition$field
  threshold_value <- criteria$conditions[[1]]$condition$value
  operator <- criteria$conditions[[1]]$condition$operator
  
  # Construct logical expression
  expr <- sprintf("df[['%s']] %s %s", field_name, operator, threshold_value)
  
  # Apply the condition and update ACMG_PM2
  df$ACMG_PM2 <- ifelse(eval(parse(text = expr)), "PM2", NA)
  
  return(df)
}

# Apply the PM2 criteria
df <- apply_acmg_pm2(df, acmg_criteria$ACMG_PM2)
...
# rm(list=setdiff(ls(), c("images_directory", "output_directory", "df",  "df_acmg", "df_acmg_caveat", "geneset_MCL_ID", "file_suffix", "output_directory", "hold", "iuis", "varsome", "ac_count_per_var")))
...
ggsave(paste(images_directory ,file_suffix, "pathogenicity_distributions_engines.pdf", sep = "") ,plot = p.pathogenicity_distributions_engines, width = 9, height = 5)
...
p.pathogenicity_distributions_engines_threshold
ggsave(paste(images_directory ,file_suffix, "pathogenicity_distributions_engines_threshold.pdf", sep = "") ,plot = p.pathogenicity_distributions_engines_threshold)
...
ggsave(paste(images_directory ,file_suffix, "pathogenicity_distributions.pdf", sep = "") ,plot = p.pathogenicity_distributions)
...
ggsave(paste(images_directory ,file_suffix, "criteria_count_each_gene.pdf", sep = "") ,plot = p.criteria_count_each_gene )
...
ggsave(paste(images_directory ,file_suffix, "criteria_gene_total.pdf", sep = "") ,plot = p.criteria_gene_total )
